...
@ContextConfiguration(classes = {ChatConsole.class, OllamaRAG.class})
// Explicitly include beans needed for ChatConsole if not covered by PresenterApplication scan
public class ChatConsoleE2ETest {
    ...
    @MockitoBean
    private CompanyHealthScorecardService companyHealthScorecardService;
    ...
    private Future<?> runChatConsoleWithInput(String input) {
        ...
        return executorService.submit(() -> chatConsole.run(new DefaultApplicationArguments(input)));
    }
    ...
    void testFullFundamentalAnalysis() throws Exception {
        ...
        when(ollamaChatModel.chat(anyList())).thenReturn(
                ChatResponse.builder()
                        .aiMessage(AiMessage.from(expectedLLMResponse))
                        .build());
